...
C = 22e-12      #capacitance(20pF)
R_main = 10e3        #resistance(10kohm)
...
fig,ax = plt.subplots()
ax.plot(t,charging(t,R_main),color="black",linewidth=2.5,label=f"R = {R_main/1e3:.0f}k$\\Omega$(main)")
for R,style in zip(R_others,styles):
    ax.plot(t,charging(t,R),color="black",linestyle=style,linewidth=1.3,label=f"R = {R/1e3:.0f}k$\\Omega$")
...
ax.legend()
fig.savefig(r"C:\Users\andre\PycharmProjects\rc-charging-poc\figures\generated\rc-charging.pdf")    #Overwrites the Stage 3 figure